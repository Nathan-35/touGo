...
class GuideSignInVC: UIViewController, UITextFieldDelegate {
    ...
    private var purpleColor = #colorLiteral(red: 0.9338732958, green: 0.4356582761, blue: 0.2996177375, alpha: 1).cgColor
    private var signInButton = ButtonPush()
    private var signUpLabel = UILabel()
    private var signUpButton = UIButton()
    private var forgotButton = UIButton()
    private var mainColor = #colorLiteral(red: 0.9338732958, green: 0.4356582761, blue: 0.2996177375, alpha: 1)
    ...
    @objc func pushSignIn() {
        ...
        let mainVC = GuideMainVC()
        navigationController?.pushViewController(mainVC, animated: true)
    }
    
    
    @objc func pushSignUp() {
        let signUpVC = GuideSignUpVC()
        ...
        if emailTextField.text!.count >= 1 && passwordTextField.text!.count >= 1 {
            UIView.animate(withDuration: 1.5) {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.7, execute: {
                    self.iconButton.setImage(UIImage(named: "man-guide"), for: .normal)
